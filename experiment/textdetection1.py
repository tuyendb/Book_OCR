...
class ImgDetector:
    ...
    def figure_remove_img(self):
        if self._figure_remove_img is None:
            ...
            if len(rects) != 0:
                for rect in rects:
                    remove_area = self._figure_remove_img[rect[1] - 5:rect[1] + rect[3] + 5, rect[0] - 5:rect[0] + rect[2] + 5]
                    mask = np.ones_like(remove_area.shape, np.uint8)
                    self._figure_remove_img[rect[1]-5:rect[1]+rect[3]+5, rect[0]-5:rect[0]+rect[2]+5] = cv2.bitwise_not(mask, mask)
            else:
                pass
...
def text_detection(image_file_list, args_paddle):
    ...
    for image_file in image_file_list:
        ...
        cropped_images, cropped_coors = ImgDetector(img, text_detector).crop2rec
        index = 0
...
def main():
    ...
    args_paddle.image_dir = "../book/gray_imgs1/image37.jpg"
